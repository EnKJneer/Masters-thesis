...
if __name__ == '__main__':
    ...
    for material in materials:
        ...
        for geometry in geometries:
            files = [f'Data/DMC60H_{material}_{geometry}_Blowhole_{version}.csv',
                     f'..\\DataSets_DMC60H_Plate_Notch_Gear/Data/DMC60H_{material}_{geometry}_Normal_{version}.csv']
            data = []
            for file in files:
                temp = pd.read_csv(file)
                if 'Blowhole' in file:
                    temp.rename(columns={'curr_x': 'curr_x_anomalie'}, inplace=True)
                data.append(temp)
            ...
            plot_time_series(df, f'{mat} {geometry}: Stromverlauf der Vorschubachse in x-Richtung',
                             f'Vergleich_{material}_{geometry}_Normal_Anomalie',
                             col_name='curr_x', label='I in A', dpi=600,
                             ycolname_1='curr_x_anomalie', ylabel_1='Anomalie')
