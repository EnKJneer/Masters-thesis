...
class PiRNN(mb.BaseTorchModel):
    ...
    def forward(self, x):
        ...
        y_pred = self.linear(x_lin) # y_physics
        return y_pred
...
class PiRNN_2(mb.BaseTorchModel):
    def __init__(
        self,
        input_size=None,
        output_size=1,
    ...
    def criterion(self, y_target, y_pred, y_mu):
        criterion = nn.MSELoss()
        return criterion(y_target.squeeze(), y_pred.squeeze()) + torch.mean(y_mu**2)
    ...
    def _initialize(self):
        ...
        self.z_layer = nn.Linear(self.n_hidden_size, self.z_dim)
        #self.mu_layer = nn.Linear(self.n_hidden_size, self.output_size)
        self.linear = nn.Linear(self.n_hidden_size, self.output_size) #
        self.to(self.device)
    ...
    def forward(self, x):
        ...
        x_pinn = x[:, :, self.idx_v]
        #x_pinn_f = x[:, :, self.idx_F]
        #x_pinn = torch.cat((x_pinn_v, x_pinn_f), dim=-1)
        if x_pinn.dim() == 2:
            x_pinn = x_pinn.unsqueeze(1)
        out, h_n = self.rnn_z(x, h0_z)  # out: [batch, seq_len, n_hidden_size]
    ...
    def predict(self, X) -> np.ndarray:
        ...
        return self.forward(X)[0].cpu().detach().numpy().squeeze()

    def train_model_inner(self, X_train, y_train, X_val, y_val, n_epochs=100, draw_loss=False, epsilon=0.00005,
                    trial=None, n_outlier=12, reset_parameters=True, patience_stop=10, patience_lr=3, **kwargs):
        ...
        for epoch in range(n_epochs):
            ...
            def closure():
                ...
                x_tensor = self.scaled_to_tensor(X_train)
                y_tensor = self.to_tensor(y_train)
                output, y_mu = self.forward(x_tensor)

                loss = self.criterion(y_tensor, output, y_mu)
                loss.backward()
                return loss
            ...
            with torch.no_grad():
                ...
                val_loss = self.criterion(y_tensor, output, y_mu)
                val_losses.append(val_loss.item())
    ...
    def train_model(self, X_train, y_train, X_val, y_val, target='curr_x', **kwargs):
        ...
        X_val_tensor = self.scaled_to_tensor(X_val)
        y_val_tensor = self.to_tensor(y_val)
        return self.train_model_inner(X_train_tensor, y_train_tensor,
            X_val_tensor, y_val_tensor,
            **kwargs)
        '''       
        # Rufe die ursprüngliche train_model-Methode auf
    ...
    def test_model(self, X, y_target, target='curr_x', **kwargs):
        ...
        # Rufe die ursprüngliche test_model-Methode auf
        return super().test_model(X, y_target, criterion_test=nn.MSELoss(), **kwargs)
