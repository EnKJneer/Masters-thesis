...
class PiRNN_unstable(mb.BaseTorchModel):
    def __init__(
        self,
        input_size=None,
        output_size=1,
    ...
    def forward(self, x):
        ...
        out, h_n = self.rnn(x_pinn, h0)  # out: [seq_len, batch, input_size]

        # Berechne z für jeden Zeitschritt
        z = self.z_layer(out)  # [seq_len, batch, input_size]
...
class PiRNN(mb.BaseTorchModel):
    ...
    def _initialize(self):
        """Initialisiert RNN, z_layer, lineare Regression und RNN-Output-Layer."""
        if self.n_hidden_size is None:
            self.n_hidden_size = self.input_size
    ...
    def _extract_physics_terms(self, x):
        """
        Extrahiere a, v, F aus dem Input-Tensor x.
        """
        if isinstance(x, torch.Tensor):
            if self.idx_v is None or self.idx_a is None or self.idx_F is None:
    ...
    def forward(self, x):
        """
        Forward-Pass: y_pred = y_linear(a,v,F) + y_rnn(z, dz_dt)
        """
        # Füge Batch-Dimension hinzu, falls nicht vorhanden
        if x.dim() == 2:
            x = x.unsqueeze(1)
    ...
    def train_model(self, X_train, y_train, X_val, y_val, **kwargs):
        ...
        # Berechne Residuen für Training
        with torch.no_grad():
            # Lineare Vorhersagen
            ...
            a_train, v_train, F_train = self._extract_physics_terms(X_train_tensor_batch)
