...
if __name__ == "__main__":
    ...
    model_gru_net = mnn.GRU(input_size, 1, input_size, 1)
    model_rnn = mnn.RNN(input_size, 1, input_size, 1)
    model_sequential = mnn.SequentialNet(input_size, 1, input_size, 1)
    model_lstm = mnn.LSTM(input_size, 1, input_size, 1)
    model_gru = mnn.GRU(input_size, 1, input_size, 1)

    models = [model_nn, model_gru_net, model_rnn, model_sequential, model_lstm, model_gru]
    ...
    for i,data in enumerate(dataSets):
        ...
        df[model_nn.name] = np.mean(nn_preds, axis=0)
        ...
        for model in models[1:]:
            ...
            rnn_preds = []
            for _ in range(NUMBEROFMODELS):
                model_rnn = mnn.RNN(input_size, 1, input_size, 1) # reset model
            ...
            # Fehlerberechnung
            n_drop_values = 10
            mse_nn, std_nn = hdata.calculate_mse_and_std(rnn_preds, y_test.values, n_drop_values)
            df[model.name] = np.mean(rnn_preds, axis=0)
            # Ergebnisse speichern
            results.extend([
    ...
    ax.set_title(f"Vergleich verschiedener Rekursiver Netze")
