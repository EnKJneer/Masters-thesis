...
import Helper.handling_data as hd
import Models.model_physical as mphys

# Define the linear function for fitting
def linear_func(x, theta0, theta1, theta2, theta3):
    a, v, f = x
    return theta0 * a + theta1 * v + theta2 * f + theta3
...
for axis in axes:
    for file in files:
        ...
        x = data[[f'v_{axis}', f'a_{axis}', f"f_{axis}_sim"]].iloc[:-n].copy()
        x.columns = [f'v_{axis}_1_current', f'a_{axis}_1_current', f"f_{axis}_sim_1_current"]

        model = mphys.FrictionModel()
        model.train_model(x, curr_axis, x, curr_axis)
        prediction = model.predict(x)

        popt, _ = curve_fit(linear_func, (a_axis, v_axis, f_axis), curr_axis)
        theta0, theta1, theta2, theta3 = popt
        # Approximation berechnen
        prediction_lin = linear_func((a_axis, v_axis, f_axis), theta0, theta1, theta2, theta3)

        plt.plot(curr_axis, label=f'curr_{axis}')
        plt.plot(prediction, label=f'Friction')
        plt.plot(prediction_lin, label=f'linear')
        plt.legend()
        plt.show()
        ...
        y_datas = [("curr_axis", curr_axis), ("pred Friction", prediction), ("pred lin", prediction_lin)]
