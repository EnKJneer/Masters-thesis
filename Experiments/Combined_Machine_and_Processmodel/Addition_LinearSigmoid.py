...
class MachineAndProcessLinearCombined(mb.BaseModel):
    def __init__(self, *args, name='MachineAndProcessModel_Linear',
                 ...
                 **kwargs):
        super(MachineAndProcessLinearCombined, self).__init__(*args, **kwargs)
        self.name = name
        ...
        self.input_acc = ['a_x_1_current', 'a_y_1_current', 'a_z_1_current', 'a_sp_1_current']
        self.input_velocity = ['v_x_1_current', 'v_y_1_current', 'v_z_1_current', 'v_sp_1_current']
        self.input_machine = self.input_acc + self.input_velocity

    def compute_machine_output(self, X):
        acc = X[self.input_acc].values
        vel = X[self.input_velocity].values
        lin = acc @ self.params_linear

        sigmoid_total = np.zeros_like(lin)
        for i, v in enumerate(self.input_velocity):
            t1 = self.params_sigmoid[i * 3 + 0]
            t2 = self.params_sigmoid[i * 3 + 1]
            t3 = self.params_sigmoid[i * 3 + 2]
            sigmoid_total += t1 / (1 + np.exp(-t2 * vel[:, i] + t3))

        return lin + sigmoid_total

    def fit_machine_model_jointly(self, X, y):
        """
        Fittet ein Maschinenmodell mit linearer Abhängigkeit von Beschleunigungen
        und sigmoidaler Abhängigkeit von Geschwindigkeiten.
        Optimiert alle Parameter gemeinsam.
        """
        n_acc = len(self.input_acc)
        n_vel = len(self.input_velocity)

        def full_machine_model(X_mat, *params):
            """
            params = [w_1, ..., w_n, theta_1_1, theta_2_1, theta_3_1, ..., theta_1_m, theta_2_m, theta_3_m]
            """
            w = np.array(params[:n_acc])  # lineare Gewichte
            theta = np.array(params[n_acc:])  # sigmoid-Parameter

            acc_data = X_mat[:, :n_acc]  # lineare Beschleunigungen
            vel_data = X_mat[:, n_acc:]  # Geschwindigkeiten

            # Linearer Teil
            linear_output = acc_data @ w

            # Sigmoid-Teil
            sigmoid_output = np.zeros_like(linear_output)
            for j in range(n_vel):
                t1 = theta[j * 3 + 0]
                t2 = theta[j * 3 + 1]
                t3 = theta[j * 3 + 2]
                sigmoid_output += t1 / (1 + np.exp(-t2 * vel_data[:, j] + t3))

            return linear_output + sigmoid_output

        # Eingabematrix vorbereiten
        X_acc = X[self.input_acc].values
        X_vel = X[self.input_velocity].values
        X_mat = np.concatenate([X_acc, X_vel], axis=1)

        # Initiale Parameter: lineare w=1, sigmoid (1, 1, 0) für jede Geschwindigkeit
        initial_params = np.concatenate([
            np.ones(n_acc),  # w
            np.tile([1.0, 1.0, 0.0], n_vel)  # theta
        ])

        # Fit durchführen
        optimal_params, _ = curve_fit(full_machine_model, X_mat, y.values.flatten(), p0=initial_params, maxfev=10000)

        # Parameter speichern
        self.params_linear = optimal_params[:n_acc]
        self.params_sigmoid = optimal_params[n_acc:]
    ...
    def predict(self, X):
        y_machine = self.compute_machine_output(X).squeeze()
    ...
    def train_model(self, X_train, y_train, X_val, y_val, n_epochs=1, trial=None, draw_loss=False, n_outlier=12, patience=10):
        ...
        for epoch in range(n_epochs):
            ...
            # Maschinenmodell gemeinsam fitten
            self.fit_machine_model_jointly(x_aircut, y_aircut)

            # Maschinenmodell auf Prozessdaten anwenden
            y_machine = self.compute_machine_output(x_process)

            # Prozessmodell trainieren
            self.model_process.fit(x_process, (y_process.squeeze() - y_machine).squeeze())
...
if __name__ == "__main__":
    ...
    hexp.run_experiment(dataClasses, use_nn_reference=False, use_rf_reference=True, models=models,
                        ...
                        plot_types=['heatmap', 'prediction_overview'], experiment_name='Addition_LinearSigmoid')