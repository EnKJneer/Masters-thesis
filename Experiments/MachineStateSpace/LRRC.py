...
def integrand_ode(z1_dot, t_curr, theta, t_data, v_data, a_data, ip_data):
    ...
    integrant = theta[4] * i10 + theta[5] * v_interp + theta[6] * a_interp
...
def residuals_with_integrals(params, args):
    ...
    z1_dot_dot = theta[7] * z1_dot_values + theta[8] * i10 + theta[9] * v + theta[10] * a
    im_model = i10 + z1_dot_values + z1_dot_dot + theta[11]
...
def smart_parameter_initialization(v, a, ip, im):
    ...
    params_init = jnp.array([
        theta_init_linear[0],  # theta[0]: v coefficient
        theta_init_linear[1],  # theta[1]: a coefficient
        theta_init_linear[2],  # theta[2]: ip coefficient
        0.1,  # theta[3]: decay rate
        0.01,  # theta[4]: integrand v coeff
        0.01,  # theta[5]: integrand a coeff
        0.01,  # theta[6]: integrand ip coeff
        0.1,  # theta[7]: z1_dot coeff
        0.01,  # theta[8]: z1_dot_dot i10 coeff
        0.01,  # theta[9]: z1_dot_dot v coeff
        0.01,  # theta[10]: z1_dot_dot a coeff
...
def run_comparison_optimized(dataClass):
    ...
    subset_factor = 1  # Use every 2nd point
    ...
    z1_dot_dot_full = theta_integrals[7] * z1_dot_values_full + theta_integrals[8] * i10_full + theta_integrals[
        9] * v_full + theta_integrals[10] * a_full
    im_model_integrals = i10_full + z1_dot_values_full + z1_dot_dot_full + theta_integrals[11]
