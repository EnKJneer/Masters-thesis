...
def residuals_nonlinear_model(params, args):
    ...
    R0, C0, R10, L10, R1, C1, x1_0, R21, L21, R2, C2, x2_0, alpha, beta, gamma, x0_0 = params
    ...
    x0 = rk4_scan_integrate(partial(integrand_x0, v_data=v), x0_0, t)
...
def run_comparison_optimized(dataClass):
    ...
    subset_factor = 4
    v = jnp.array(X_train['v_x_1_current'][:-n:subset_factor])
    a = jnp.array(X_train['a_x_1_current'][:-n:subset_factor])
    ip = jnp.array(X_train['f_x_sim_1_current'][:-n:subset_factor])
    tau_measured = jnp.array(y_train['curr_x'][:-n:subset_factor])  # Hier nehmen wir curr_x als tau
    ...
    R0, C0, R10, L10, R1, C1, x1_0, R21, L21, R2, C2, x2_0, alpha, beta, gamma, x0_0 = theta_nonlinear

    # i10 berechnen
    i10_full = v_full / R0 + a_full / C0 + ip_full * gamma

    # x0 berechnen, numerische Integration
    x0_full = rk4_scan_integrate(partial(integrand_x0, v_data=v_full), x0_0, t_full)

    # x1 berechnen (analytische Lösung)
    exponential_term_1_full = jnp.exp(-R10 / L10 * t_full)
    x1_full = x0_full + (x1_0 + R10 * i10_full) * exponential_term_1_full

    # v1 berechnen (Ableitung von x1)
    v1_full = jnp.gradient(x1_full, t_full[1] - t_full[0])

    # i21 berechnen
    i21_full = v1_full / R1 + jnp.gradient(v1_full, t_full[1] - t_full[0]) / C1 + i10_full

    # x2 berechnen (analytische Lösung)
    exponential_term_2_full = jnp.exp(-R21 / L21 * t_full)
    x2_full = x1_full + (x2_0 + R21 * i21_full) * exponential_term_2_full

    # v2 berechnen (Ableitung von x2)
    v2_full = jnp.gradient(x2_full, t_full[1] - t_full[0])

    # im berechnen
    im_full = v2_full / R2 + jnp.gradient(v2_full, t_full[1] - t_full[0]) / C2 + i21_full
    ...
    # Stufe 1: i10, x1 und v
    ax2.plot(t_full, i10_full, label="i10", color='green', linewidth=1.5)
    ax2.plot(t_full, x1_full, label="x1", color='orange', linewidth=1.5)
    ax2.plot(t_full, v_full, label="v (Input)", color='gray', alpha=0.7)
    ax2.set_xlabel("Zeit (Samples)")
    ax2.set_ylabel("Werte")
    ax2.legend()
    ax2.set_title("Stufe 1: i10, x1 und Eingangssignal v")
    ax2.grid(True)

    # Stufe 2: i21, x2 und v1
    ax3.plot(t_full, i21_full, label="i21", color='purple', linewidth=1.5)
    ax3.plot(t_full, x2_full, label="x2", color='brown', linewidth=1.5)
    ax3.plot(t_full, v1_full, label="v1", color='pink', alpha=0.8)
    ax3.set_xlabel("Zeit (Samples)")
    ax3.set_ylabel("Werte")
    ax3.legend()
    ax3.set_title("Stufe 2: i21, x2 und v1")
    ax3.grid(True)

    # Parameter-Darstellung
    param_names = ['R0', 'C0', 'R10', 'L10', 'R1', 'C1', 'x1_0', 'R21', 'L21', 'R2', 'C2', 'x2_0', 'alpha', 'beta', 'gamma', 'x0_0']
    colors = ['red', 'blue', 'green', 'orange', 'purple', 'brown', 'pink', 'gray', 'cyan', 'magenta', 'yellow', 'lime', 'navy', 'maroon', 'teal', 'black']
    ...
    print(f"x1: {jnp.mean(x1_full):.4f} ± {jnp.std(x1_full):.4f}")
    print(f"i21: {jnp.mean(i21_full):.4f} ± {jnp.std(i21_full):.4f}")
    print(f"x2: {jnp.mean(x2_full):.4f} ± {jnp.std(x2_full):.4f}")
