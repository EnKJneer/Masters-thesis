    ...
    if self.add_sign_y:
        ...
        for x_df, y_df in zip(X, Y):
            for col in y_df.columns:
                ...
                x_df[y_col_sign] = y_df[col].shift(1).apply(lambda x: 0 if pd.isna(x) else np.sign(x)).fillna(0)
...
if __name__ == '__main__':
    ...
    dataclass1.name = 'ohne sign y'
    dataclass2 = copy.copy(dataSet)
    dataclass2.name = 'mit sign y'
    ...
    hexp.run_experiment(dataClasses, models=models,
                        ...
                        plot_types=['heatmap', 'prediction_overview'], experiment_name='Sign_embedding')
