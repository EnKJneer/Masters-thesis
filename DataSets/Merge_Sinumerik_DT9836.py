...
def mse_random_forest_offset(df_target, df_source, target_cols, source_cols, max_shift):
    ...
    offsets = range(-max_shift, max_shift + 1, 10)
    scores = [calculate_regression_mse(offset, X_target, y_source, 2) for offset in offsets]
...
# Laden der Daten
path_data_sin = 'DataSinumerik_50Hz'
path_data_dt = 'DataDT9836_50Hz'
output_path = 'DataMerged'
...
for file in files:
    if file.endswith('.csv'):
        ...
        if file.startswith('S235JR') or file.startswith('AL_2007_T4_Gear_Depth'):
