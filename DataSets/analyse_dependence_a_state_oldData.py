...
for axis in axes:
    for file in files:
        ...
        for y_data in y_datas:
            ...
            ax.scatter(a_axis, y_data[1], c=color_value[1], s=2, alpha=0.5, label=label)
            ax.set_xlabel(f'a_{axis}')
            ax.set_ylabel(y_data[0])

            dx = []
            dy = []
            for i in range(len(v_axis) - 1):
                dx.append(a_axis[i + 1] - a_axis[i])
                dy.append(y_data[1][i + 1] - y_data[1][i])
            ax.quiver(a_axis[:-1], y_data[1][:-1], dx, dy, angles='xy', scale_units='xy', scale=1, width=0.005,
