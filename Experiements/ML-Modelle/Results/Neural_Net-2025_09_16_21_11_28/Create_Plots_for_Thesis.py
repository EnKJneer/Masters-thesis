...
class BasePlotter(ABC):
    ...
    def replace_space(text):
        ...
        ergebnis = ergebnis.replace('Neural', 'Künstliches\nneuronales')
...
class HeatmapPlotter(BasePlotter):
    ...
    def create_plots(self, df: pd.DataFrame = None, title: str = 'Model Vergleich', model_names=None, **kwargs):
        ...
        ax.set_title(f'MAE Heatmap:\nKünstliches neuronales Netz', fontsize=titlesize, fontweight='bold', pad=20, color=self.kit_dark_blue)
...
def calculate_mae_and_std(df, datapath, ground_truth_column='GroundTruth', model_prefixes=None):
    ...
    if model_prefixes is None:
        model_prefixes = [
            'ST_Plate_Notch_Neural_Net_RandomSampler',
            'ST_Plate_Notch_Neural_Net_GridSampler',
            'ST_Plate_Notch_Neural_Net_TPESampler'
        ]
