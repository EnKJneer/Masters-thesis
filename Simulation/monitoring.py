...
def state_monitoring(machine_state: ms.MachineState, tool: vc.Tool, part: vc.PartialVoxelPart,
    ...
    for i in range(process_data.shape[0]):
        ...
        a_e, angle = calculate_ae_and_angle_for_tool(tool, part_position, part_dimension)

        process_data.loc[i, 'a_p'] = a_p
        process_data.loc[i, 'a_e'] = a_e
        process_data.loc[i, 'cut_angle'] = angle

        current_process_state = ms.ProcessState(pos_x, pos_y, pos_z, v_x, v_y, v_z, v_sp, a_p, tool.radius*2, 180)
        ...
        if materialremoved_sim > mrr_mean/10 and a_p > 1 and a_e > 1:
    ...
    if plot_mrr:
        ...
        plt.title('a_p')
        plt.show()
        plt.plot(process_data['cut_angle'])
        plt.title('cut_angle')
        plt.show()
