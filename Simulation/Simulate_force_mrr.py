...
def get_part_properties(material_geometry, df=None):
    ...
    if material == "S235JR":
        ...
        if "Gear" in geometry:
            ...
            part_dimension = [70, 70, 50, 0.1]
        elif "workpiece1" in geometry:
            part_position = [-37.8, 269.27, 339.69]
            part_dimension = [70, 70, 50, 0.1]
        elif "workpiece2" in geometry:
            part_position = [0.8*10**6, 0.8*10**6, 339.69]
            part_dimension = [70, 70, 50, 0.1]
...
if __name__ == '__main__':
    ...
    show_results = False
    #path = 'DataSet_TrainingValidation_1\DataSinumerik_50Hz'
    #path_target = 'DataSet_TrainingValidation_1\Data'
    ...
    for file in files:
        ...
        if file.endswith('aircut') or 'Aircut' in file:
            # Berechnung nicht nötig
            for axis in ['x', 'y', 'z']: 
                new_col_name = f'f_{axis}_sim'
                if new_col_name not in raw_data.columns:
                    raw_data[new_col_name] = 0  # Neuer Spaltenname mit 0 gefüllt
            file += '.csv'
            path_data = os.path.join(path_target, file)
            raw_data.to_csv(path_data)

            continue
        
        material_geometry = file.replace('DMC60H_', '').replace('CMX600V_', '')
